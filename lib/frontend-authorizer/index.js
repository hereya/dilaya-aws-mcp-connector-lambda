...
// A direct hit on app.dilaya.eu can't reproduce it. When configured, the
// authorizer requires it — but TRANSITIONALLY also accepts the legacy marker, so
// the distribution's origin-header config can propagate to every edge with no 403
// window. The strict-cutover release removes the marker fallback (see below).
...
exports.handler = async function (event) {
  try {
    ...
    if (APP_CONTENT_DOMAIN && SITE_OR_AUTH_RE.test(rawPath)) {
      const h = event.headers || {};
      const secretOk =
        !!ORIGIN_VERIFY_SECRET && h[ORIGIN_VERIFY_HEADER] === ORIGIN_VERIFY_SECRET;
      // TRANSITIONAL marker fallback — remove in the strict-cutover release once
      // logs confirm `secretOk` requests are arriving from every edge. When no
      // secret is configured at all this IS the gate (feature-off / 0.1.25 parity).
      const markerOk = !!h[EDGE_MARKER_HEADER];
      if (!(secretOk || markerOk)) return { isAuthorized: false };
      if (ORIGIN_VERIFY_SECRET) {
        // Observability for the cutover: shows whether the secret header has
        // propagated (secretOk:true everywhere) before we drop the marker fallback.
        console.log(
          "origin-lock",
          JSON.stringify({ path: rawPath, secretOk, markerOk })
        );
      }
    }
