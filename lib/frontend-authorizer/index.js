...
//
// For an app WITHOUT `authEnforce` on its registry row it returns
// `isAuthorized: true` whatever the session (mirroring the legacy authorizer):
// the per-app handler decides whether an anonymous request is acceptable or
// must be redirected to `/auth/login`. "Fail closed" there means: on ANY error
// or missing session we grant NO identity (empty email/cognito_sub) — never a
// forged one.
//
// For an app WITH `authEnforce` (enable-auth enforce:true / a site born
// private — t_frontend_auth_default, deploy-pkg >= 0.1.73) the PLATFORM closes
// the site: an anonymous request for a site path outside the app's declared
// `publicPaths` (and outside /static/*) is REFUSED (403), whatever the handler
// would have done. The edge router already sends cookie-less visitors to
// /auth/login; this is the guard behind it (a forged or expired cookie gets
// here, and gets nothing). Fail closed all the way: an enforced app whose pool
// row cannot be read, or whose registry row cannot be read, is refused too.
//
...
const { makeFrontDoor, isFrontDoorEvent } = require("./front-door");
const {
...
/** The app row's publicPaths attribute (a JSON string array, written by the
 *  connector's registry/app-access.ts). Garbled → no public path, which on an
 *  enforced app closes MORE, never less. */
function parsePublicPaths(raw) {
  if (!raw) return [];
  try {
    const arr = JSON.parse(raw);
    if (Array.isArray(arr)) return arr.filter((x) => typeof x === "string" && x.startsWith("/"));
  } catch (err) {
    // fall through
  }
  return [];
...
/**
 * Resolve an app name to its row: { appId, enforce, publicPaths } — null when
 * the app is missing or being deleted. THROWS on a registry error (the caller
 * decides what a blip means; on an enforced site it must not mean "open").
 */
// The last app row resolveApp read, for the front door (front-door.js): it
// needs the same row to find the function, and one invocation handles exactly
// one request, so re-reading it would only double the registry round-trips.
let lastAppRow = null;

async function resolveApp(orgId, app) {
  ...
  lastAppRow = { orgId, app, item: appRow.Item };
  return {
    appId,
    enforce: appRow.Item.authEnforce === true,
    publicPaths: parsePublicPaths(appRow.Item.publicPaths),
  };
}

// App-relative path of a SITE request: `/o/<org>/<app>/site[-stg]<rest>`
// → `<rest>` (`/` when empty). Null for anything else (the auth tree, the
// public agent/telegram routes) — those are never gated here.
const SITE_PATH_RE = /^\/o\/[^/]+\/[^/]+\/site(?:-stg)?(\/.*)?$/;
function siteRelativePath(rawPath) {
  const m = (rawPath || "").match(SITE_PATH_RE);
  return m ? m[1] || "/" : null;
}

/**
 * Is this request one the platform must refuse when anonymous? Only site
 * paths of an ENFORCED app, minus /static/* (assets — the login page's own
 * logo lives there) and the app's declared public prefixes ('/' = the root
 * page only). Same reading as the edge router's branch, so the two agree on
 * every path.
 */
function gatedPath(rawPath, access) {
  if (!access || !access.enforce) return false;
  const uri = siteRelativePath(rawPath);
  if (uri === null) return false;
  if (uri === "/static" || uri.startsWith("/static/")) return false;
  for (const p of access.publicPaths || []) {
    if (p === "/" ? uri === "/" : uri === p || uri.startsWith(p + "/")) return false;
  }
  return true;
}
...
async function readAuthConfig(orgId, app) {
  ...
  // A registry error THROWS out of here (see the handler: a site path is then
  // refused, never served open).
  const row = await resolveApp(orgId, app);
  if (!row) return null; // app missing/deleting
  const value = {
    appId: row.appId,
    enforce: row.enforce,
    publicPaths: row.publicPaths,
    userPoolId: null,
    userPoolClientId: null,
    poolUnreadable: false,
  };
  try {
    ...
    if (rec) {
      ...
      if (poolId) {
        value.userPoolId = poolId;
        value.userPoolClientId = fieldStr(rec[1]);
      }
    }
  } catch (err) {
    // Table absent / VM unreachable → "auth not resolvable": anonymous on a
    // handler-guarded app, REFUSED on an enforced one. Never cached, so a
    // transient blip self-heals.
    console.error("frontend-authorizer: _auth_config lookup failed:", err?.message || err);
    value.poolUnreadable = true;
    return value;
  }
...
// --- Monthly request cap (the one that CUTS) -------------------------------
//
// Two brakes now sit on this path and they answer different questions. The
// per-IP guard below is about a RUNAWAY: one address looping, throttled for a
// minute, back to normal on its own. This one is about the BILL: an
// organization consuming beyond what its plan includes, for a whole month.
//
// Cutting a customer's site is the heaviest thing in this file, and it was
// argued: I put the case against it (a monthly cap that bites takes a working
// site off the air until the 1st of the next month, and it lands on the
// customer whose site SUCCEEDS). Jonatan's answer settled it — "il faut couper
// au plafond quitte à laisser 5% de marge, sinon le risque financier est sur
// moi". Unbounded exposure is worse than a bounded outage, and it is his
// exposure. The mitigation is not to skip the cut but to make it rare: the
// projection-and-warning half is a separate piece of work, so that a customer
// is told at 50-60% of the month's allowance and never discovers this line.
//
// THE 5% MARGIN is his too, and it does real work: a month's counter is an
// approximation (best-effort writes, a request counted that never rendered),
// so cutting at exactly 100% would cut some customers slightly early on a
// number that is not exact to the request. 105% is past any plausible drift
// and still bounded.
const REQUEST_CAP_MARGIN = 1.05;

const orgMonthPk = (orgId, month) => `reqcountorg#${orgId}#${month}`;

// The org's plan cap, as the connector cached it from dilaya.eu (org-info).
// Cached here for a minute, exactly like the per-app auth config above and for
// the same reason: this runs on every request, and the number changes about
// once in a customer's lifetime.
const orgCapCache = new Map(); // orgId → { at, cap }
const ORG_CAP_TTL_MS = 60 * 1000;

/**
 * The org's monthly request allowance, or null for "no cap".
 *
 * `null` is UNLIMITED and is honoured as such — the same rule that governs
 * every other cap on this platform, and the one whose misreading capped
 * Dilaya's own admin org for ten minutes on 2026-08-28. An unreadable registry
 * also yields null: a lookup we cannot make must never be the reason a site
 * goes dark.
 */
async function orgRequestCap(orgId) {
  const hit = orgCapCache.get(orgId);
  if (hit && Date.now() - hit.at < ORG_CAP_TTL_MS) return hit.cap;
  let cap = null;
  try {
    if (REGISTRY_TABLE) {
      const res = await ddb.send(
        new GetCommand({
          TableName: REGISTRY_TABLE,
          Key: { org_id: orgId, sk: 'org' },
        })
      );
      const v = res.Item && res.Item.maxRequestsMonth;
      cap = typeof v === "number" && Number.isFinite(v) && v > 0 ? v : null;
    }
  } catch (err) {
    console.error("frontend-authorizer: org cap lookup failed:", err?.message || err);
    return null; // never cache a failure as "capped"
  }
  orgCapCache.set(orgId, { at: Date.now(), cap });
  return cap;
}

/**
 * Count this request against the org's month and say whether it is past the
 * cap.
 *
 * ONE atomic ADD reading its own result — the same shape as the per-app
 * counter and the rate guard. It is deliberately a SECOND counter rather than
 * a sum of the per-app rows: a cap belongs to the organization, and summing N
 * apps would mean N reads on every single request.
 *
 * Best-effort in the safe direction: any failure returns "not over", so a
 * counter we cannot write never takes a site off the air.
 */
async function checkMonthlyCap(orgId) {
  if (!APP_STATE_TABLE) return { over: false, count: 0, cap: null };
  const cap = await orgRequestCap(orgId);
  const month = new Date().toISOString().slice(0, 7);
  try {
    const res = await ddb.send(
      new UpdateCommand({
        TableName: APP_STATE_TABLE,
        Key: { pk: orgMonthPk(orgId, month) },
        UpdateExpression: "ADD requests :one SET expires_at = :e",
        ExpressionAttributeValues: {
          ":one": 1,
          // Thirteen months, like the per-app counter: long enough to compare a
          // month with the same month last year.
          ":e": Math.floor(Date.now() / 1000) + 400 * 24 * 3600,
        },
        // ALL_NEW, not UPDATED_NEW: the item also carries `edge_requests`,
        // written by the connector from the CloudFront access log. Reading it
        // here costs nothing extra — it rides back on the write this path
        // already does.
    ...
    //
    // `requests` is what THIS authorizer has counted: requests that reached the
    // origin. `edge_requests` is what CloudFront's own log says it served for
    // this org, which is a SUPERSET — a cache hit and every path of a
    // static-mode site are answered at the edge and never arrive here at all,
    // so until 2026-08-29 they were counted by nothing and this cap could not
    // reach them (t_b8f659db595c).
    //
    // Summing would count every dynamic request twice and cut customers at
    // half their allowance. The maximum also makes the log's lag harmless: the
    // real-time count governs until the complete one overtakes it, and the
    // figure can only ever move up towards the truth.
    const live = Number(res.Attributes?.requests ?? 0);
    const edge = Number(res.Attributes?.edge_requests ?? 0);
    const count = Math.max(
      Number.isFinite(live) ? live : 0,
      Number.isFinite(edge) ? edge : 0
    );
    // The count is kept even when there is no cap: it is what the projection
    // and the usage report read, and an org that is uncapped today may not be
    // tomorrow.
    if (cap === null) return { over: false, count, cap: null };
    return { over: count > cap * REQUEST_CAP_MARGIN, count, cap };
  } catch (err) {
    console.error("frontend-authorizer: monthly cap check failed:", err?.message || err);
...
// ENFORCING by default since 2026-08-28. It shipped in count mode that morning
// and the counted data is not what flipped it — Jonatan asked three times for a
// real brake and was right to: a guard that refuses nothing leaves the platform
// exactly as exposed as before. The margin is what makes enforcing safe: 1000
// per minute is 2.6x the busiest legitimate visitor-minute ever measured (388)
// and comfortably under the runaway (1155), so the population it can cut is
// one that no real visitor has ever come close to.
//
// `FRONTEND_RATE_BLOCK=false` is the OFF switch, and it is deliberately the
// default's inverse rather than a deploy parameter: a package parameter has TWO
// halves (declared in hereyarc.yaml, substituted in hereyavars/) and forgetting
// either one deploys GREEN while doing nothing — the exact silent failure of
// 2026-08-07. A default in code cannot half-arrive.
const RATE_BLOCK = String(process.env.FRONTEND_RATE_BLOCK || "") !== "false";

// THE LONG WINDOW (t_rate_guard_sustained, 2026-09-13). The minute guard above
// has a floor it cannot go under — a real visitor's busiest minute is 388 —
// and the 2026-09-13 loop lived under it: one browser, ~300 requests a minute
// to one app's /api/auth/me, 3 800 in twenty minutes, never once near 1 000.
// What the minute cannot see, the quarter-hour can: a person bursts and stops
// (388, then 270, then nothing), a loop keeps going. Measured on 14 days of
// CloudFront logs (2026-08-30 → 09-13, 163 580 origin-bound requests — cache
// hits and edge redirects never reach this function and were left out),
// requests per (host, viewer ip, quarter-hour):
//
//     browser loops on /api/auth/me (one address, one app):
//        cariaco 2026-09-03   1 894 · 7 524 · 7 590 · 2 137 · 2 226
//        komlaba 2026-09-13   3 045 · 4 961 · 5 951
//     vulnerability scanners (curl, fake bingbot):   up to 2 718
//     everything else — i.e. every real visitor:      under 963
//
// So 2 000 per quarter-hour: over twice anything a person has ever done,
// under every loop bucket but the one where a loop is still warming up.
// Same request, same table, same atomic ADD — one more row per (app, ip,
// window), four an hour instead of sixty.
//
// COUNT MODE BY DEFAULT, unlike the minute guard, and on purpose: a longer
// window is where a shared address (a school's NAT, a mobile carrier gateway)
// accumulates — the population it can wrongly cut is bigger than the minute's.
// So it reports first (`rate_guard` line with `window` set → the same metric
// filter → the same alarm), and FRONTEND_RATE_WINDOW_BLOCK=true turns the
// brake on once the counted lines say who it would actually cut.
// FRONTEND_RATE_WINDOW_LIMIT=0 removes the counter entirely.
const RATE_WINDOW_MINUTES = Math.max(
  1,
  Math.min(60, Number(process.env.FRONTEND_RATE_WINDOW_MINUTES || 15) || 15)
);
const RATE_WINDOW_LIMIT = Number(process.env.FRONTEND_RATE_WINDOW_LIMIT ?? 2000);
const RATE_WINDOW_BLOCK =
  String(process.env.FRONTEND_RATE_WINDOW_BLOCK || "") === "true";
...
const rateWindowPk = (orgId, app, tag, windowStart) =>
  `ratewin#${orgId}#${app}#${tag}#${windowStart}`;

/** Start of the tumbling window holding `now`, as YYYY-MM-DDTHH:MM. A loop
 *  that straddles two windows is caught in the second one at the latest. */
function windowStartOf(now) {
  const floored = Math.floor(now.getUTCMinutes() / RATE_WINDOW_MINUTES) * RATE_WINDOW_MINUTES;
  return now.toISOString().slice(0, 14) + String(floored).padStart(2, "0");
}

/** One atomic ADD on a counter row, reading its own result. Best-effort:
 *  any failure reads as "not over", because a counter we cannot write must
 *  never be the reason a working page is refused. */
async function bumpCounter(pk, ttlSeconds) {
  ...
  return Number(res.Attributes?.hits ?? 0);
}
...
async function checkRate(orgId, app, ip) {
  ...
  try {
    ...
    const count = await bumpCounter(rateCountPk(orgId, app, ipTag(ip), minute), 300);
...
/**
 * Count this request against its (app, ip, window) bucket — the long window
 * that separates a loop from a person by DURATION. Same contract as checkRate:
 * atomic, best-effort, never the reason a working page is refused.
 */
async function checkWindow(orgId, app, ip) {
  if (!APP_STATE_TABLE || !ip || !(RATE_WINDOW_LIMIT > 0)) {
    return { over: false, count: 0 };
  }
  try {
    const pk = rateWindowPk(orgId, app, ipTag(ip), windowStartOf(new Date()));
    const count = await bumpCounter(pk, RATE_WINDOW_MINUTES * 60 + 300);
    return { over: count > RATE_WINDOW_LIMIT, count };
  } catch (err) {
    console.error("frontend-authorizer: window check failed:", err?.message || err);
    return { over: false, count: 0 };
  }
}

/** The `rate_guard` line — one per offending request, shaped as a stable
 *  metric-filter target. `window` says which counter tripped ("1m" or the
 *  long one), `blocked` what actually happened, so the same line reads
 *  correctly in both modes and the alarm never has to change. */
function logRateGuard(orgId, app, ip, rawPath, window, hits, limit, blocked) {
  console.warn(
    JSON.stringify({
      type: "rate_guard",
...
// The platform's refusal (API Gateway answers 403). Only ever returned for a
// SITE path of an enforced app — see gatedPath.
...
async function authorize(event) {
  ...
  try {
    ...
    if (APP_CONTENT_DOMAIN && SITE_OR_AUTH_RE.test(rawPath)) {
      ...
      if (!ok) {
        // One line per refusal (t_origin_lock_log): it used to leave no trace
        // but the access log — 18 refusals of a new customer went unseen.
        const who = extractOrgApp(rawPath);
        console.warn(
          JSON.stringify({
            type: "origin_lock_denied",
            org: who?.orgId ?? null,
            app: who?.app ?? null,
            marker: !!h[EDGE_MARKER_HEADER],
            verify: !!h[ORIGIN_VERIFY_HEADER],
    ...
    // The month's allowance, checked before the per-minute guard: an org past
    // its plan is refused whatever its instantaneous rate looks like.
    const month = await checkMonthlyCap(orgId);
    if (month.over) {
      console.warn(
        JSON.stringify({
          type: "request_cap",
    ...
    // Both counters on the same request, in parallel — the long window adds
    // a row, not a round-trip. Each is logged on its own line when it trips.
    const [rate, win] = await Promise.all([
      checkRate(orgId, app, ip),
      checkWindow(orgId, app, ip),
    ]);
    if (rate.over) {
      logRateGuard(orgId, app, ip, rawPath, "1m", rate.count, RATE_LIMIT_PER_MINUTE, RATE_BLOCK);
    }
    if (win.over) {
      logRateGuard(
        orgId, app, ip, rawPath, `${RATE_WINDOW_MINUTES}m`,
        win.count, RATE_WINDOW_LIMIT, RATE_WINDOW_BLOCK
      );
    }
    if ((rate.over && RATE_BLOCK) || (win.over && RATE_WINDOW_BLOCK)) {
      return { isAuthorized: false };
    }

    let cfg;
    try {
      cfg = await readAuthConfig(orgId, app);
    } catch (err) {
      // Registry unreadable: we cannot tell an enforced site from a public
      // one, so a site path is refused rather than served open. The auth
      // tree and the other routes keep the legacy anonymous pass-through.
      console.error("frontend-authorizer: registry lookup failed:", err?.message || err);
      return siteRelativePath(rawPath) === null ? ANON : DENY;
    }
    if (!cfg) return ANON; // app missing/deleting
    const gated = gatedPath(rawPath, cfg);
    // Enforced, and nothing to verify a session against (no pool yet — a site
    // born private whose enable-auth is still running or failed — or a pool
    // row we could not read): closed, not open.
    if (gated && !cfg.userPoolId) return DENY;
    if (!cfg.userPoolId) return ANON; // auth not enabled, the handler decides
    ...
    // 3. Anonymous — refused on an enforced site path (t_frontend_auth_default);
    //    elsewhere the per-app handler redirects to /auth/login if it must.
    if (gated) {
      console.log(
        JSON.stringify({
          type: "auth_enforced",
          ...
          app,
          path: rawPath.slice(0, 200),
          reason: idToken ? "invalid_session" : agentCookie ? "invalid_agent_session" : "anonymous",
        })
      );
      return DENY;
    }
...
// The same function answers two callers: API Gateway's authorizer (a REQUEST
// event, legacy per-app routes) and the static front-door routes (an
// integration event) — see front-door.js.
const frontDoor = makeFrontDoor({
  authorize,
  appRow: async (orgId, app) => {
    const hit = () => (lastAppRow && lastAppRow.orgId === orgId && lastAppRow.app === app ? lastAppRow.item : null);
    if (hit()) return hit();
    await resolveApp(orgId, app);
    return hit();
  },
  authFunctionName: process.env.AUTH_FUNCTION_NAME || "",
  appFunctionPrefix: process.env.APP_LAMBDA_NAME_PREFIX || "",
});

exports.handler = async function (event) {
  lastAppRow = null;
  return isFrontDoorEvent(event) ? frontDoor(event) : authorize(event);
};
