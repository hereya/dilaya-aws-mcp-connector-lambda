...
const APP_TOO_SLOW = () => reply(504, "Gateway Timeout");

// How long the whole front-door request may take before we stop waiting for
// the app (t_front_door_app_timeout, 24/09). API Gateway cuts at 30 s and so
// does this function's own timeout: an app slower than that used to kill the
// sandbox mid-invoke — no log line naming the app, Errors=1 on the authorizer
// (its alarm then counts a tenant's slowness), a mute 503 for the visitor. So
// we give up first, answer a 504 and say which app it was. The app's own
// invocation keeps running to its end: a synchronous Lambda invoke cannot be
// cancelled from the caller's side.
const APP_DEADLINE_MS = 28_000;
...
function makeFrontDoor(deps) {
  ...
  const invoke = (input, abortSignal) => {
    const sdk = require("@aws-sdk/client-lambda");
    if (!lambda) lambda = new sdk.LambdaClient({ maxAttempts: 1 });
    return lambda.send(new sdk.InvokeCommand(input), { abortSignal });
  };
  const deadlineMs = deps.appDeadlineMs || APP_DEADLINE_MS;
  ...
  return async function frontDoor(event) {
    ...
    // The budget counts from the request's arrival: authorize + the registry
    // lookup already spent part of API Gateway's 30 s.
    const abort = new AbortController();
    let timedOut = false;
    const timer = setTimeout(() => {
      timedOut = true;
      abort.abort();
    }, Math.max(0, deadlineMs - (started - entered)));
    const tooSlow = new Promise((resolve) => abort.signal.addEventListener("abort", () => resolve(null)));
    let res;
    try {
      // Raced as well as aborted: the answer must not depend on the SDK
      // honouring the signal promptly.
      ...
      // An abort surfaces as the SDK's AbortError: that is the timeout below.
      if (!timedOut) {
        const name = err?.name || "";
    ...
    if (timedOut) {
      console.error(JSON.stringify({ type: "front_door_app_timeout", org: target.orgId, app: target.app, kind: target.kind, ms: Date.now() - started, totalMs: Date.now() - entered, deadlineMs }));
      return APP_TOO_SLOW();
    }
