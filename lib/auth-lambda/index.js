...
// in an href. Without WebAuthn the page skips itself; a device that already
...
<body>
  ...
  (function(){
    ...
    yes.addEventListener('click',function(){
      ...
      post("passkey/register/start",{}).then(function(st){
        ...
        if(cred.authenticatorAttachment)body.authenticatorAttachment=cred.authenticatorAttachment;
        return post("passkey/register/finish",{credential:body});
      }).then(function(f){
...
async function handleVerify(event, ctx) {
  ...
  try {
    ...
    if (result.AuthenticationResult?.IdToken) {
      ...
      const cookies = [idCookie(orgId, app, idToken, 86400, appHost), lastEmailCookie(orgId, app, email, appHost)];
...
async function handlePasskeyFinish(event, ctx) {
  ...
  try {
    ...
    return jsonResponse(200, { redirect: safeReturnUrl(params.return_url, orgId, app, appHost) }, [
      idCookie(orgId, app, idToken, 86400, appHost),
