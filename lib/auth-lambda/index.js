...
//      branding: custom_css / login_title / logo_url / lang / accent_color) and the
...
const i18n = {
  en: {
    ...
    otpSubtitle: (email) => `If <strong>${email}</strong> has access to this app, a 6-digit code has just been sent to it.`,
    ...
    otpThrottled: "Too many codes were requested for this address. Use the last code you received, or try again in 15 minutes.",
  ...
  fr: {
    ...
    otpSubtitle: (email) => `Si <strong>${email}</strong> a accès à cette application, un code à 6 chiffres vient de lui être envoyé.`,
    ...
    otpThrottled: "Trop de codes ont été demandés pour cette adresse. Utilisez le dernier code reçu, ou réessayez dans 15 minutes.",
...
// The app's stored language wins (t_login_page_lang_brand, 13/09/2026: a French
// app's custom title sat over English platform strings on an English phone);
// the browser decides only when the app never said.
function pickLang(event, branding) {
  const l = branding && branding.lang;
  if (l === "fr" || l === "en") return l;
  return detectLang(event);
}
function t(lang) {
...
// ONE color → the accent variables the shared stylesheet reads, derived here so
// EVERY page follows — including the OTP page's secondary "resend" button that
// hand-written custom_css kept forgetting. Strict #rrggbb: the value lands
// inside <style>, so nothing else may pass.
function accentStyle(color) {
  if (typeof color !== "string" || !/^#[0-9a-f]{6}$/i.test(color)) return "";
  const hex = color.toLowerCase();
  const [r, g, b] = [1, 3, 5].map((i) => parseInt(hex.slice(i, i + 2), 16));
  const clamp = (v) => Math.max(0, Math.min(255, Math.round(v)));
  const toHex = (...c) => "#" + c.map((v) => clamp(v).toString(16).padStart(2, "0")).join("");
  const hover = toHex(r * 0.85, g * 0.85, b * 0.85);
  const soft = toHex(255 - (255 - r) * 0.1, 255 - (255 - g) * 0.1, 255 - (255 - b) * 0.1);
  return `:root{--accent:${hex};--accent-hover:${hover};--accent-soft:${soft};--accent-ring:rgba(${r},${g},${b},.15)}`;
}
function brandStyle(b) {
  const a = accentStyle(b && b.accentColor);
...
const SHARED_STYLE = `
    :root { --accent: #4f46e5; --accent-hover: #4338ca; --accent-soft: #f5f3ff; --accent-ring: rgba(79,70,229,.1); }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .back-link { display: block; text-align: center; margin-top: 16px; font-size: 0.85rem; color: var(--accent); text-decoration: none; }
...
<head>
  ...
  ${brandStyle(b)}${b.customCss ? `<style>${b.customCss}</style>` : ""}
</head>
<body>
  <div class="card">
    ${logo}
...
// --- Decoy sessions (t_login_enumeration, 2026-09-13) ----------------------
//
// The login page used to answer "No account found for this email" — a
// different page for an unknown address than for a known one, i.e. an oracle
// listing a customer's users address by address (Jonatan, 13/09, screenshot).
// Now EVERY address gets the same code page. For an address that gets no code
// (not allowlisted, unknown to Cognito, auth not enabled) the page carries a
// DECOY session: random bytes whose tail is an HMAC under a key derived from
// the capability secret, the same length as the real ones, so verify can
// recognise it without storing anything and answer "incorrect code" — exactly
// what a wrong code on a real session gets. A small jitter stands in for the
// Cognito + Postmark round-trips the real path makes, so the two answers are
// not told apart by the clock either (not a proof, a narrowing). Without a
// capability secret (local runs) the key is per-process: a decoy verified on
// another container falls through to Cognito and reads as an expired session.
const DECOY_BODY_BYTES = 600;
const DECOY_TAG_BYTES = 32;
let decoyKeyCache;
async function decoyKey() {
  if (decoyKeyCache) return decoyKeyCache;
  const secret = await getCapabilitySecret();
  decoyKeyCache = secret
    ? crypto.createHmac("sha256", secret).update("decoy-session-v1").digest()
    : crypto.randomBytes(32);
  return decoyKeyCache;
}
async function decoySession() {
  const body = crypto.randomBytes(DECOY_BODY_BYTES);
  const tag = crypto.createHmac("sha256", await decoyKey()).update(body).digest().subarray(0, DECOY_TAG_BYTES);
  return Buffer.concat([body, tag]).toString("base64");
}
async function isDecoySession(session) {
  if (typeof session !== "string") return false;
  let buf;
  try {
    buf = Buffer.from(session, "base64");
  } catch (err) {
    return false;
  }
  if (buf.length !== DECOY_BODY_BYTES + DECOY_TAG_BYTES) return false;
  const body = buf.subarray(0, DECOY_BODY_BYTES);
  const tag = crypto.createHmac("sha256", await decoyKey()).update(body).digest().subarray(0, DECOY_TAG_BYTES);
  return crypto.timingSafeEqual(tag, buf.subarray(DECOY_BODY_BYTES));
}
function decoyDelay(minMs, spanMs) {
  return new Promise((r) => setTimeout(r, minMs + Math.random() * spanMs));
}
/** The code page for an address that gets no code — indistinguishable from
 *  the real one but for the (decoy) session it carries. */
// --- OTP send throttle (t_quota_mail_bypass) --------------------------------
// Every send-otp is a Postmark mail. Counted per (org, app, ADDRESS) in 15-min
// windows, BEFORE the allowlist is consulted: counting only known addresses
// would make "too many codes" an oracle over the app's user list. Fails OPEN —
// an unwritable counter must never keep a person out of their own app.
const OTP_SENDS_PER_WINDOW = 5;
const OTP_WINDOW_S = 900;

async function otpSendAllowed(orgId, app, email) {
  const table = process.env.APP_STATE_TABLE;
  if (!table) return true;
  const bucket = Math.floor(Date.now() / 1000 / OTP_WINDOW_S);
  const tag = crypto.createHash("sha256").update(email.toLowerCase()).digest("hex").slice(0, 16);
  try {
    const res = await ddb.send(
      new UpdateCommand({
        TableName: table,
        Key: { pk: `otpsend#${orgId}#${app}#${tag}#${bucket}` },
        UpdateExpression: "ADD n :one SET expires_at = :e",
        ExpressionAttributeValues: { ":one": 1, ":e": (bucket + 2) * OTP_WINDOW_S },
        ReturnValues: "UPDATED_NEW",
      })
    );
    return Number(res?.Attributes?.n ?? 0) <= OTP_SENDS_PER_WINDOW;
  } catch (err) {
    console.error("auth-lambda: otp throttle unreadable:", err?.name || err?.message || err);
    return true;
  }
}

async function decoyOtpPage(app, reason, email, returnUrl, notice, branding, lang) {
  console.log(JSON.stringify({ type: "auth_no_code", ts: new Date().toISOString(), app, reason }));
  const session = await decoySession();
  await decoyDelay(250, 400);
  return htmlResponse(200, otpPage(session, email, returnUrl, null, notice, branding, lang));
}
...
async function handleLogin(event, ctx) {
  ...
  const lang = pickLang(event, branding);
...
async function handleSendOtp(event, ctx) {
  ...
  // Past the window's allowance NO mail leaves, known address or not — and the
  // page is the same one every address gets, notice aside.
  if (!(await otpSendAllowed(orgId, app, email))) {
    return decoyOtpPage(app, "otp_throttled", email, returnUrl, s.otpThrottled, branding, lang);
  }

  const appAuth = await resolveAppAuth(orgId, app);
  if (!appAuth) {
    // Auth not enabled / app not resolvable — fail closed, and say nothing.
    return decoyOtpPage(app, "auth_not_enabled", email, returnUrl, notice, branding, lang);
  }

  // Allowlist gate (best-effort; null = couldn't check → let Cognito decide).
  // An address that is not on the list gets the SAME page as one that is.
  const allowed = await isAllowlisted(orgId, appAuth.appId, email);
  if (allowed === false) {
    return decoyOtpPage(app, "not_allowlisted", email, returnUrl, notice, branding, lang);
  }
  ...
  try {
    ...
    if (err.name === "UserNotFoundException" || err.name === "NotAuthorizedException") {
      return decoyOtpPage(app, "unknown_to_pool", email, returnUrl, notice, branding, lang);
    }
...
async function handleVerify(event, ctx) {
  ...
  // A decoy session (an address that got no code) answers exactly what a
  // wrong code on a real session answers — Cognito is never asked.
  if (await isDecoySession(session)) {
    await decoyDelay(80, 150);
    return htmlResponse(200, otpPage(session, email, returnUrl, s.incorrectCode, null, branding, lang));
  }
...
async function handlePasskeyStart(event, ctx) {
  ...
  // An address off the allowlist answers like one without a passkey: the
  // client falls back to the e-mail code, which says the same thing to everyone.
  const allowed = await isAllowlisted(orgId, appAuth.appId, email);
  if (allowed === false) return jsonResponse(200, { fallback: "no_passkey" });
  try {
...
async function handlePasskeyRegisterPage(event, ctx) {
  ...
  const branding = await resolveBranding(orgId, app);
  return htmlResponse(200, registerPage(location, branding, pickLang(event, branding)));
}
...
// HEAD routes like the GET it mirrors (HTTP: same answer, no body — the HTTP
// layer drops it). Without this `HEAD /auth/login` answered 404 while GET said
// 200, so an uptime monitor saw a live login page as down. Logout is excluded:
// a safe method must not clear the session.
function routeMethod(method, action) {
  return method === "HEAD" && action !== "logout" ? "GET" : method;
}
...
exports.handler = async function (event) {
  ...
  const method = routeMethod(event.requestContext?.http?.method || "GET", ctx.action);
