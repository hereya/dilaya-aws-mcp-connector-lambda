...
//   3. Reads the app's `_auth_config` (user_pool_client_id / from_email) and the
  ...
  try {
    ...
    if (appId) {
      const res = await dataApiQuery(
        orgId,
        appId,
        "SELECT user_pool_client_id, from_email FROM _auth_config WHERE id = 1"
      );
      const rec = res.records && res.records[0];
      if (rec) {
        const clientId = fieldStr(rec[0]);
        const fromEmail = fieldStr(rec[1]);
        if (clientId) value = { appId, clientId, fromEmail };
      }
    }
  } catch (err) {
...
// Only allow local, single-slash redirect targets.
function safeReturnUrl(raw, orgId, app) {
  if (raw && raw.startsWith("/") && !raw.startsWith("//")) return raw;
  return sitePrefix(orgId, app);
}

// Cookie scoped to this app's path so app A's session is never sent to app B
// (defence in depth — the authorizer also validates against A's pool).
function idCookie(orgId, app, value, maxAge) {
...
async function handleLogin(event, ctx) {
  ...
  if (cookies[ID_TOKEN_COOKIE] && isUnexpiredJwt(cookies[ID_TOKEN_COOKIE])) {
    return { statusCode: 302, headers: { Location: safeReturnUrl(returnUrl, orgId, app) }, body: "" };
  }

  const css = await loadCustomCss(orgId, app);
...
async function handleSendOtp(event, ctx) {
  ...
  if (!email) return htmlResponse(400, loginPage(returnUrl, s.emailRequired, css, lang, ""));
  ...
  try {
    ...
    return htmlResponse(
      200,
      otpPage(result.Session, email, returnUrl, null, isResend ? s.codeResent : null, css, lang)
    );
  } catch (err) {
...
async function handleVerify(event, ctx) {
  ...
  try {
    ...
    if (result.AuthenticationResult?.IdToken) {
      ...
      return {
        statusCode: 302,
        headers: { Location: safeReturnUrl(returnUrl, orgId, app) },
        cookies: [idCookie(orgId, app, idToken, 86400)],
        body: "",
      };
    }
    if (result.Session) {
      return htmlResponse(200, otpPage(result.Session, email, returnUrl, s.incorrectCode, null, css, lang));
    }
    return htmlResponse(500, loginPage(returnUrl, s.authFailed, css, lang, email));
  } catch (err) {
    console.error("auth-lambda: RespondToAuthChallenge error:", err?.name || err?.message || err);
    return htmlResponse(200, otpPage(session, email, returnUrl, s.expiredSession, null, css, lang));
  }
}
...
async function handleLogout(event, ctx) {
  ...
  return {
    statusCode: 302,
    headers: { Location: `/o/${orgId}/${app}/auth/login` },
    cookies: [idCookie(orgId, app, "", 0)],
    body: "",
  };
}
...
exports.__test__ = { parsePath, safeReturnUrl, idCookie, mintCapability, b64url };
